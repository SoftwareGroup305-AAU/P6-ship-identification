...
class ConvBlock(nn.Module):
    ...
    def forward(self, images):
        return self.silu(self.bn(self.conv(images)))
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        objectness_output = torch.sigmoid(self.objectness(x)).squeeze(1)
        localization_output = torch.sigmoid(self.localization(x)).permute(0, 2, 3, 1).contiguous()
